...
大量一覧では表示範囲と前後の行、選択行をDOMに残し、省いた行の高さを空白で保つ。
行高は実際の行から測り、ページ全体のスクロールと列幅を維持する。
全件のデータをソート・選択・一括操作の対象とし、画面内の行だけに範囲を狭めない。
支援技術へ全件数と各行の位置を公開し、選択先のDOMは画面外でも保持する。
...
大量一覧の回帰確認では同じ隔離データを5,000作品へ増やす。
初回表示1,500ms未満、最大long task 750ms未満を検証する。
入力から2回目のanimation frameまでは150ms未満とする。
この閾値はChromium 153、1440×900、Ryzen 7 PRO 4750GEで再現した劣化から定めた。
1,000作品の本棚・500作品のブランドでも履歴・対象ID・一括対象件数と狭幅への変更を確認する。
