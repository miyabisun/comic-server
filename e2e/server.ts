...
function reset(count = 100) {
	...
	for (let id = 1; id <= count; id++) {
		...
		db.insert(comics).values({ id, file, title: id === 2 ? '長い作品名'.repeat(30) : `作品 ${String(id).padStart(3, '0')}`,
			brand: id <= count / 2 ? 'Zeta' : 'Alpha', bookshelf: 'unread', created_at: new Date(Date.UTC(2026, 8, 11, 0, 0, count - id)).toISOString() }).run();
...
app.post('/__reset', c => {
	...
	if (!Number.isInteger(count) || count < 1 || count > 20_000) return c.text('invalid count', 400);
	reset(count);
	return c.text('ok');
});
